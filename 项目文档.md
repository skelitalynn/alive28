...
• 每日打卡铸造 1 个 DaySBT（不可转让）；28 天完成后可合成 1 个 FinalSBT
...
• badgeType：DaySBT 对应 dayIndex(1..28)；FinalSBT 为单独最终徽章
...
为便于 Demo 不必等待 28 天，允许在开发环境启用演示模式：
...
• Milestone：每日完成 -> mint DaySBT；累计 28 个 DaySBT -> 合成 FinalSBT
...
￮ 完成 28 天：合成 **FinalSBT** + 生成一张可分享海报（只显示徽章与统计，不泄露内容）
...
• 规则调整（MVP 变更）：
  - 每日完成打卡后铸造 1 个 DaySBT（不可转让）
  - 当累计 28 个 DaySBT 后，允许合成 1 个 FinalSBT

• 不可转让：SBT（transfer 禁用/拦截）

• mint 触发：前端点击触发（用户自己发交易）；后端仅做资格判断

**合成机制细节（规范）**

SBT 铸造链上交互不进入 Graph Agent 自动执行链路：前端发交易；后端仅提供 shouldMintDay/shouldComposeFinal，并通过 /sbt/confirm 记录结果用于进度展示。

**合约函数（MVP 规范）**
- ProofRegistry：submitProof(uint16 dayIndex, bytes32 proofHash)
- RestartBadgeSBT（ERC721, SBT）：
  - mintDay(uint8 dayIndex) external
    - require(dayIndex in 1..28)
    - require(proofRegistry.getProof(msg.sender, dayIndex) != 0)
    - require(!dayMinted[msg.sender][dayIndex])
    - tokenId = uint256(keccak256(abi.encodePacked(msg.sender, dayIndex)))
    - dayMinted=true；dayMintCount += 1
  - composeFinal() external
    - require(dayMintCount[msg.sender] == 28)
    - require(!finalMinted[msg.sender])
    - require(1..28 天 proof 都存在) // 逐日检查 proofRegistry.getProof(msg.sender, i) != 0
    - tokenId = uint256(keccak256(abi.encodePacked(msg.sender, uint8(99))))
    - finalMinted = true

**最简策略（MVP 写死）**
- 不 burn 28 个 DaySBT（避免复杂枚举/销毁）
- 合成 FinalSBT 只检查“28 天 proof 都存在 + 28 个 DaySBT 已 mint”，并设置 finalMinted=true
• 不使用 Ownable/onlyOwner，允许用户自行 mint/compose

**前端流程（上链交互）**
1) 用户完成每日打卡 -> 后端生成 proofHash
2) 前端调用 ProofRegistry.submitProof(dayIndex, proofHash)
3) 交易成功后，前端调用 RestartBadgeSBT.mintDay(dayIndex)
4) 当进度显示已完成 28 天时，显示“合成 FinalSBT”按钮
5) 点击后调用 RestartBadgeSBT.composeFinal()

**后端校验点（MVP）**
- /checkin：不负责 mint，仅生成 proofHash 和进度
- /progress：返回 streak/已完成天数；合成资格可基于本地日志判断
- /tx/confirm：仅记录 proof txHash
- /sbt/confirm：前端在 mintDay/composeFinal 成功后回传，后端更新 day_mint_count / final_minted

TODO:
- 是否需要后端对 DaySBT/FinalSBT 的链上状态进行校验
...
• **out**：{streak, completedDays, dayMintCount, finalMinted, shouldMintDay, mintableDayIndex, shouldComposeFinal}
...
• **in**：{address, streak, todayCheckedIn, todayDayMinted, dayMintCount, finalMinted}

• **out**：{shouldMintDay, mintableDayIndex, shouldComposeFinal}

￮ mintableDayIndex：1..28（对应 DaySBT）
...
**6.2 RestartBadgeSBT.sol（ERC721 + SBT：DaySBT & FinalSBT）**

**核心目标**

• DaySBT：每日完成后 mint 1 个（共 28 个，对应 dayIndex 1..28）

• FinalSBT：当累计 28 个 DaySBT 后允许合成 1 个

• SBT：禁止 transfer（只允许 mint/burn；MVP 不 burn）
...
• mapping(address => uint8) public dayMintCount; // 0..28

• mapping(address => bool) public finalMinted;

**tokenId 规则（MVP 写死）**

• DaySBT tokenId：uint256(keccak256(abi.encodePacked(user, dayIndex)))

• FinalSBT tokenId：uint256(keccak256(abi.encodePacked(user, uint8(99)))) // 99 作为 final 标记
...
• dayCount(address user) external view returns (uint8);

• finalOf(address user) external view returns (bool);
...
• transfer hook 限制：只允许 from==0（mint）或 to==0（burn）

• 任何 from!=0 && to!=0 均 revert
...
**DaySBT metadata（示例）**
{
  ...
  "attributes": [
    ...
    {"trait_type": "DayIndex", "value": 6},
    {"trait_type": "Challenge", "value": "Alive28"}
  ]
}

**FinalSBT metadata（示例）**
{
  "name": "Alive28 - Final",
...
• address public immutable sbtContract;：关联的 SBT 合约地址，用于核实挑战完成情况。
...
￮ **逻辑**：用户完成 28 天挑战并获得最高等级 SBT 后，调用此函数取回 0.001 ETH。

￮ **校验**：调用 RestartBadgeSBT.finalOf(msg.sender) == true 或 dayMintCount == 28。
...
| day_mint_count | int           | 否         |               | 0               | 已铸造 DaySBT 数量（0..28）            |
| final_minted   | bool          | 否         |               | false           | 是否已合成 FinalSBT                    |
| final_sbt_tx_hash | char(66)     | 是         |               |                 | FinalSBT 合成交易回传（可选）           |
...
| day_sbt_tx_hash  | char(66)             | 是               |                     |                  | DaySBT 铸造交易回传（可选）                          |
...
• UserProgress 取 streak/last_date_key/day_mint_count/final_minted
...
JSON {  "dateKey":"2026-01-31",  "task":{ "dayIndex":7, "title":"...", "instruction":"...", "hint":"..." },  "log":{ "id":"uuid", "address":"0x...", "challengeId":1, "dayIndex":7, "dateKey":"2026-01-31", "normalizedText":"...", "reflection":{"note":"...","next":"..."}, "saltHex":"0x...", "proofHash":"0x...", "status":"CREATED", "txHash":null, "daySbtTxHash":null, "createdAt":"2026-01-31T12:00:00Z" },  "alreadyCheckedIn":true }
...
JSON {  "log":{ "id":"b5a1f7c7-9df7-4f31-8b3b-3c7a0a9c3e1d", "address":"0xabc...", "challengeId":1, "dayIndex":6, "dateKey":"2026-01-29", "normalizedText":"...", "reflection":{ "note":"你今天确实很累，但还把这一步做完了。", "next":"现在用1分钟写下：我最想保住的边界是__。" }, "saltHex":"0x...", "proofHash":"0x1234...abcd", "status":"CREATED", "txHash":null, "daySbtTxHash":null, "createdAt":"2026-01-29T10:10:00Z" }, "alreadyCheckedIn":false }
...
JSON {  "dateKey":"2026-01-31",  "streak":4,  "dayMintCount":4,  "completedDays":[1,2,3,4,6],  "shouldMintDay":true,  "mintableDayIndex":6,  "shouldComposeFinal":false,  "finalMinted":false,  "finalSbtTxHash":null,  "milestones":{"1":null,"2":null,"3":null},  "startDateKey":"2026-01-25" }
...
JSON {  "address":"0xabc...",  "type":"DAY",  "dayIndex":6,  "txHash":"0x...",  "chainId":11155111,  "contractAddress":"0xsbt..." }

type 枚举："DAY" | "FINAL"

当 type="DAY"：必须带 dayIndex

当 type="FINAL"：不带 dayIndex
...
DAY：
- 若 day_mint_count < 28 且该 dayIndex 未被确认过，则 day_mint_count += 1
- 写入当日 DailyLog.day_sbt_tx_hash

FINAL：
- final_minted = true
- 写入 UserProgress.final_sbt_tx_hash

幂等（写死）：
- DAY：以 DailyLog(address, challenge_id, day_index) 为定位；若该记录已存在 day_sbt_tx_hash，则直接返回 {"ok":true} 不重复累计。
- FINAL：若 UserProgress.final_minted == true，则直接返回 {"ok":true} 不重复写入。

说明：DailyLog 写入时必须保证 (address, challenge_id, day_index) 逻辑上唯一（通过查询保证，不强制 DB UNIQUE）。
7.9 **报告（周报/结营）**
...
JSON {  "title":"周报（模拟）",  "reportText":"这一周你完成了4天打卡…",  "recentLogs":[ { "id":"uuid", "address":"0x...", "challengeId":1, "dayIndex":7, "dateKey":"2026-01-31", "normalizedText":"...", "reflection":{"note":"...","next":"..."}, "saltHex":"0x...", "proofHash":"0x...", "status":"CREATED", "txHash":null, "daySbtTxHash":null, "createdAt":"2026-01-31T12:00:00Z" } ],  "chartByDay":[0,1,0,2,0,0,1,...],  "range":"week" }
...
• 建议报告只基于 reflection 与统计，不使用 raw input（更隐私友好）
...
5. POST /tx/confirm
6. 合约 mintDay(dayIndex)
7. POST /sbt/confirm (type=DAY, dayIndex, txHash)
8. GET /progress?address=...

D.2 结营流程（第 28 天后）
1. GET /progress -> shouldComposeFinal=true
2. 合约 composeFinal()
3. POST /sbt/confirm (type=FINAL, txHash)
4. GET /report?range=final
...
用途：为 DaySBT / FinalSBT 提供最简 metadata（可先返回固定占位图）。

Response 200

JSON metadata（见 6.2 的示例）。
...
• **OpenZeppelin Contracts**（ERC721，用于 SBT 改造）
