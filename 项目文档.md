...
• **后端**：DailyLog 增加 day_sbt_tx_hash；POST /sbt/confirm（type=DAY 写回当日 day_sbt_tx_hash 并递增 day_mint_count，type=FINAL 写回 final_sbt_tx_hash 与 final_minted）；GET /progress 返回 dayMintCount、shouldMintDay、mintableDayIndex、shouldComposeFinal、finalMinted、finalSbtTxHash；DailyLogResponse 含 daySbtTxHash。

• **合约**：ProofRegistry（submitProof）、RestartBadgeSBT（mintDay、composeFinal）、MilestoneNFT（mint(to, tokenId, uri)，ERC721URIStorage）；Deploy.s.sol 部署上述三个合约。

• **前端**：已调用 POST /tx/confirm、POST /sbt/confirm、POST /milestone/mint；上链流程为打卡 → 保存记录 → 上链提交（ProofRegistry）→ 完成今日（RestartBadgeSBT.mintDay + /sbt/confirm）；结营为 RestartBadgeSBT.composeFinal + /sbt/confirm；里程碑为 MilestoneNFT.mint + /milestone/mint。配置 NEXT_PUBLIC_PROOF_REGISTRY、NEXT_PUBLIC_BADGE_SBT、NEXT_PUBLIC_MILESTONE_NFT 后即可上链。

• **数据库**：若已有 alive.db，需删除重建或执行 ALTER TABLE dailylog ADD COLUMN day_sbt_tx_hash VARCHAR(66); 以支持 Day SBT 记录。
